...
export class ReflectionTexture extends CubeTexture {
    constructor(engine, background, quality, alpha) {
        ...
        this.engine.addOnTexturesLoaded(() => {this.generate()})
    }

    generate () {
        console.log('Reflect textures generating')
        ...
        let posXP = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha })
        camera.setRotation(0, 90, 0);
        let posXN = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha });
        camera.setRotation(-90, 0, 0);
        let posYP = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha });
        camera.setRotation(90, 0, 0);
        let posYN = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha });
        camera.setRotation(0, 0, 0);
        let posZP = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha });
        camera.setRotation(0, 180, 0);
        let posZN = this.engine.captureFrame(camera, { backgroundColor: this.background, width: this.quality, height: this.quality, imageAlpha: this.alpha });
        if (this.alpha < 1) {
            this.alpha = true
        }
        ...
        console.log('Reflect textures generating end')
    }
